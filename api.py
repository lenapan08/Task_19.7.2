...
class PetFriends:
    ...
    def post_add_new_pet(self, auth_key: json, name: str, animal_type: str, age: str, pet_photo: str) -> json:
        """Метод дбавляет информацию на сервере о новом питомце и возвращает статус и результат в формате json
с данными добавленнного питомца"""
    ...
    def delete_pet(self, auth_key: json, pet_id: str) -> json:
        """"Метод позволяет отправить запрос на удаление питомца с определенным ID ,результатом является ответ
        с успешным статусом и подтверждением в виде Json, что питомец удален"""
    ...
    def put_update_pet_info(self, auth_key: json, pet_id: str, name: str, animal_type: str, age: str) -> json:
        """Метод позволяет внести изменения в информацию о питомце с определенным ID,ответ возвращается в виде успешого
    статуса и result  в формате json с внесенными изменениями"""
...
с данными добавленнного питомца"""
    ...
    def post_add_photo_of_pet(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод позволяет добавить фото питомца с определенным ID,ответ возвращается в виде успешого
    статуса и result  в формате json с внесенными изменениями"""
